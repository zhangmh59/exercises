...
int main(int argc, char* argv[])
{
	...
	void main()
	{
		...
		if(gl_VertexID == 0){vColor = vec3(1,0,0);gl_Position =  modelMatrix * vec4(0,0 ,0,1);}
		if(gl_VertexID == 1){vColor = vec3(0,1,0);gl_Position =  modelMatrix * vec4(1,0 ,0,1);}
		if(gl_VertexID == 2){vColor = vec3(0,0,1);gl_Position =  modelMatrix * vec4(0,1 ,0,1);}
